    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-700 text-white font-sans">
      ...
      <div className="lg:hidden fixed top-4 right-4 z-50">
        <button
          ...
          className="bg-yellow-500 hover:bg-yellow-400 text-black p-3 rounded-full shadow-lg transition-all duration-300 hover:scale-105"
        >
        ...
        <div className="flex-1">
          <section className="relative py-12 sm:py-16 md:py-20 lg:py-32 bg-gray-900 overflow-hidden">
            ...
            <div className="absolute inset-0 pointer-events-none">
              <div className="absolute top-0 left-0 w-64 sm:w-80 md:w-96 h-64 sm:h-80 md:h-96 bg-yellow-500/10 rounded-full blur-3xl transform -translate-x-1/2 -translate-y-1/2"></div>
              <div className="absolute bottom-0 right-0 w-64 sm:w-80 md:w-96 h-64 sm:h-80 md:h-96 bg-red-500/10 rounded-full blur-3xl transform translate-x-1/2 translate-y-1/2"></div>
            </div>
            ...
            <div className="container mx-auto px-4 sm:px-6 md:px-8 lg:px-20 relative z-10">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8 md:gap-10">
                {menuItems.map((item) => (
                  <div
                    key={item.id}
                    className={`group relative bg-white/5 backdrop-blur-lg border rounded-2xl sm:rounded-3xl p-4 sm:p-6 md:p-8 
                  ...
                  >
                    ...
                    <div className="relative z-10">
                      <h3 className={`font-serif text-lg sm:text-xl md:text-2xl mb-3 sm:mb-4 font-semibold transition-colors duration-300 leading-tight
                        ${item.available 
                      ...
                      <p className={`leading-relaxed mb-4 sm:mb-6 text-sm sm:text-base transition-colors duration-300
                        ${item.available 
                      ...
                      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0">
                        <div className={`text-xl sm:text-2xl font-semibold transition-all duration-300
                          ${item.available 
                        ...
                        {item.available ? (
                          <button
                            ...
                            className="bg-yellow-500 hover:bg-yellow-400 text-black px-4 py-2 rounded-lg font-semibold text-sm transition-all duration-300 hover:scale-105 shadow-lg hover:shadow-xl w-full sm:w-auto"
                          >
                            Add to Cart
                          </button>
                        ) : (
                          <button
                            disabled
                            className="bg-gray-600 text-gray-400 px-4 py-2 rounded-lg font-semibold text-sm cursor-not-allowed w-full sm:w-auto"
                          >
                            Unavailable
                          </button>
                        )}
      ...
      {/* Mobile Cart Overlay */}
      {showCart && (
        <div className="lg:hidden fixed inset-0 bg-black/50 backdrop-blur-sm z-40" onClick={() => setShowCart(false)}>
          <div className="absolute right-0 top-0 h-full w-full max-w-sm bg-gray-900 shadow-2xl transform transition-transform duration-300">
            <div className="p-4 h-full flex flex-col">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-xl font-serif text-yellow-400 font-bold">Your Cart</h3>
                <button
...
const CartComponent = ({ 
  ...
  return (
    <div className={`bg-gray-800/90 backdrop-blur-lg border border-gray-700/50 rounded-2xl p-4 sm:p-6 shadow-2xl w-full h-full flex flex-col ${isMobile ? 'max-h-none' : 'max-h-[600px]'}`}>
      <div className="mb-4">
        {!isMobile && (
      ...
      {selectedItems.length === 0 ? (
        ...
        <div className="flex flex-col h-full">
            ...
            {selectedItems.map((item) => (
              <div key={item.id} className="bg-white/5 rounded-xl p-3 border border-gray-700/30">
                <div className="flex justify-between items-start mb-2">
                  <h4 className="text-yellow-400 font-semibold text-xs leading-tight pr-2">{item.name}</h4>
                  <button
          ...
          {/* Order Summary */}
          <div className="border-t border-gray-700/50 pt-4 space-y-3 flex-shrink-0">
